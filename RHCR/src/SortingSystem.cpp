...
void SortingSystem::update_goal_locations(int t)
{
    ...
    for (int k = 0; k < num_of_drives; k++)
    {
        ...
        int last_goal_loc = std::get<0>(goal);
        ...
        // The agent might finish its tasks during the next planning horizon
        {
            ...
            min_timesteps += G.get_Manhattan_distance(std::get<0>(next), last_goal_loc);
            goal = next;
        }
    }
...
void SortingSystem::update_n_agents(tuple<int, int, int> task)
{
    if (this->G.types[get<1>(task)] == "Workstation")
    {
        this->robots_in_workstations[get<1>(task)]--;
    }
    else if (this->G.types[get<1>(task)] == "Endpoint")
    {
        this->robots_in_endpoints[get<1>(task)]--;
    }
    else
    {
        cout << "task is at loc " << get<1>(task) << endl;
        cout << "task is of type " << this->G.types[get<1>(task)] << endl;
