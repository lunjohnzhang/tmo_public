...
void BeeSystem::update_goal_locations()
{
	...
	{
		tuple<int, int, int> curr(paths[k][timestep].location, timestep, 0); // current location
		...
		int min_timesteps = max(G.get_Manhattan_distance(std::get<0>(curr), std::get<0>(goal)), // cannot use h values, because graph edges may have weights
			std::get<1>(goal) - timestep);
			...
			// The agent might finish its tasks during the next planning horizon
		{
			...
			goal_locations[k].emplace_back(
                make_tuple(next.first, next.second, 0));
			min_timesteps += G.get_Manhattan_distance(next.first, std::get<0>(goal));
			min_timesteps = max(min_timesteps, std::get<1>(goal) - timestep);
