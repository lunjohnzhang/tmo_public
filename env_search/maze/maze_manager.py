...
class MazeManager:
    ...
    def emulation_pipeline(self, sols):
        ...
        success_mask = np.ones(len(envs), dtype=bool)
        objs, measures = self.emulation_model.predict(envs)
        return envs, objs, measures, success_mask
    ...
    def eval_pipeline(self, sols, parent_sols=None, batch_idx=None):
        ...
        futures = [
            self.client.submit(
                ...
                seed,
                pure=False,
            ) for env, seed in zip(envs, evaluation_seeds)
        ]
    ...
    def add_experience(self, sol, result):
        ...
        input_env = result.maze_metadata["level"]
        if self.emulation_model.pre_network is not None:
            self.emulation_model.add(
                AugExperience(sol, input_env, obj, meas,
