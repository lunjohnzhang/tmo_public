...
AugExperience = namedtuple(
    "AugExperience", Experience._fields + ("aug_env",))

# Used for batches of items, e.g. a batch of envs, a batch of objectives.
...
class AugBuffer(Buffer):
    ...
    def add(self, e: AugExperience):
        ...
        super().add(e)
        self.aug_envs.append(e.aug_env)
    ...
    def to_tensors(self):
        ...
        return BatchExperience(
            torch.as_tensor(np.array(self.envs),
                            device=DEVICE,
                            dtype=torch.float),
            ...
            torch.as_tensor(np.array(self.measures),
                            device=DEVICE,
                            dtype=torch.float),
            torch.as_tensor(np.array(self.aug_envs),
                            device=DEVICE,
                            dtype=torch.float),
        )
