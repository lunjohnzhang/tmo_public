...
class MazeEmulationModel:
        ...
        for _ in range(self.train_epochs):
            ...
            for sample in dataloader:
                if self.pre_network is None:
                    ...
                    inputs, objs, measures, aug_envs = sample
                ...
                if self.pre_network is not None:
                    pred_pre_envs = self.pre_network.int_to_logits(inputs)
                    pre_loss = pre_loss_func(nn.Flatten()(pred_pre_envs),
                                             nn.Flatten()(aug_envs))
                    # pre_loss = pre_loss_func(pred_pre_envs, aug_envs.long())
    ...
    def predict_with_grad(self, inputs: torch.Tensor):
        ...
        aug_envs = None
        if self.pre_network is not None:
            aug_envs = self.pre_network.int_to_logits(inputs)
        return self.network.predict_objs_and_measures(inputs, aug_envs)
