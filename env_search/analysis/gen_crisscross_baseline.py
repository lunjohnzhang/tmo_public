...
def gen_crisscross_baseline(
    ...
    if domain == "kiva":
        ...
        base_map_str, map_name = read_in_sortation_map(map_filepath)
        base_map_np = sortation_env_str2number(base_map_str)

    # sortation has the same order of direction
    ...
    if domain == "kiva":
        ...
        edge_weights_compress = comp_compress_edge_matrix(
            base_map_np, edge_weights_matrix.flatten(), block_idxs)
        wait_cost_compress = comp_compress_vertex_matrix(
            base_map_np, wait_costs_matrix.flatten(), block_idxs)
        breakpoint()
    raw_weights = [
        *wait_cost_compress,
    ...
    write_map_str_to_json(
        ...
        weight=True,
        weights=raw_weights,
        # optimize_wait=True if domain == "sortation" else False,
    )
