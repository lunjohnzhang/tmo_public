...
def collect(manifest: str,
    ...
    metric_names = {
        ...
        "Best Objective": "Best Throughput",
        "Mean Evaluation": "Batch Avg Throughput",
        # "Actual QD Score (Result Archive)": "QD-score",
        # "Similarity Score of Env w/ Best Objective": "Sim Score of Best Env"
    }
...
def comparison(
    ...
    for env in figure_data:
        ...
        if "Maze" in env:
            plot_every = 1000
        elif "Kiva" in env or "Manufacture" in env or "Sort" in env:
            plot_every = 50
        ...
        with mpl_style_file("comparison.mplstyle") as f, plt.style.context(f):
            ...
            for (row_val, col_val), ax in grid.axes_dict.items():
                ...
                if "Maze" in env:
                    ...
                    if col_val == "QD-score":
                        ...
                        ax.set_ylim(0, 0.3)
                        if env == "Kiva_PIBT_extinct_33x36":
            ...
            for extension in ["pdf", "png", "svg"]:
                grid.fig.savefig(
                    output /
                    f"{slugify.slugify(env)}.{extension}",
                    dpi=300,
                    bbox_inches='tight',
                )
