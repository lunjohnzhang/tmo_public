...
def load_archive_from_history(logdir: LogDir,
                              individual=False) -> GridArchive:  # type: ignore
    ...
    if archive_type == "@GridArchive":
        ...
        archive = GridArchive(solution_dim=10, seed=42, dtype=np.float64)
    else:
...
def load_surrogate_archive(
    ...
    if archive_type == "@GridArchive":
        # Same manufacture as in Manager.
        # pylint: disable = no-value-for-parameter
        surrogate_archive = GridArchive(seed=42, dtype=np.float64)
    else:
        raise TypeError(f"Cannot handle archive type {archive_type}")
    ...
    if len(downsample_archive_paths) > 0:
        downsample_archive = GridArchive(seed=42, dtype=np.float64)
...
def load_archive_from_reload(logdir: LogDir, search_algo, is_cma_mae=True):
    with open(logdir.pfile("reload.pkl"), "rb") as file:
        data = pkl.load(file)

        if search_algo in [SearchAlgo.EM, SearchAlgo.EXTINCT]:
...
def get_color(map_from, algo_name, n_agents_opt=None):
    # if map_from in ["Two-stage Opt"]:
    #     return "blue"
    # elif map_from in ["Direct Co-opt"]:
    #     return "red"
    if map_from in [
            "Chute capacity search (200 agents)",
            "CCS 200 agents",
            "CCS 200 agents, No Guidance",
            "CCS 800 agents",
    ]:
        return "green"
    elif map_from in ["Chute capacity search (600 agents)"]:
        return "pink"
    elif map_from in ["Random Chute Mapping"]:
        return "red"
    elif map_from == "MIQP Baseline":
        return "blue"
    elif map_from in [
            "GA 200 agents",
            "GA 200 agents, No Guidance",
            "GA 800 agents",
    ]:
        return "purple"
    elif map_from == "GA 600 agents":
        return "orange"
    elif map_from in [
            "Online GGO + GA 200 agents",
            "Fix GA, Train G-Policy 200 agents",
            "Fix GA, Train G-Policy 800 agents",
    ]:
        return "pink"
    elif map_from in [
            "Online GGO + CCS 200 agents",
            "Fix CCS, Train G-Policy 200 agents",
    ]:
        return "blue"
    elif map_from == "Co-opt (CCS, G-Policy) 200 agents":
        return "orange"
    elif map_from in [
            "Co-opt (G-Policy, Task Policy) + CCS 200 agents",
            "Fix CCS, Co-opt (G-Policy, Task Policy)",
            "Fix CCS, Co-opt (G-Policy, T-Policy), TA=f(H, Num Agents)",
            "Optimize CCS CM, Then DCM",
    ]:
        return "purple"
    elif map_from in [
            "Co-opt (G-Policy, Task Policy) + GA 200 agents",
            "Fix GA, Co-opt (G-Policy, Task Policy)",
            "Optimize GA CM, Then DCM",
    ]:
        return "orange"
    elif map_from in [
            "Online GGO + CCS 200 agents, Heuristic Task Assign",
            "Fix CCS, Train G Policy 800 agents, TA=Heuristic",
    ]:
        return "green"
    elif map_from in [
            "Fix CCS, Train G-Policy 800 agents",
            "Online GGO + CCS 800 agents",
            "Fix CCS, Train G-Policy 800 agents, TA=Heuristic+Num Agents",
    ]:
        return "dodgerblue"
    elif map_from in [
        "Co-opt (CSS, G-Policy, T-Policy), TA=f(H, Num Agents)",
        "Co-opt (CSS, DCM)",
    ]:
        return "tab:blue"
    elif map_from in [
        "One-by-one opt (CSS, G-Policy, T-Policy), TA=f(H, Num Agents)",
    ]:
        return "tab:orange"

    # if map_from in ["DSAGE (alpha=5)", "DSAGE", "Optimized Layout"]:
    #     if algo_name == "RHCR":
    ...
    # ]:
    #     return "indigo"
    # elif map_from == "CMA-ME + NCA":
    #     return "cyan"
    # elif map_from in [
    #         "CMA-MAE + NCA (Hamming obj)",
    #         "CMA-MAE + NCA (alpha=1)",
    #         "QD-Extinct",
    # ]:
    #     return "red"
...
def get_extreme_pt(archive_df: pd.DataFrame):
    """
    Get the "extreme" points in the archive. i.e. choose the largest measure1 
    or measure2. If tie, choose the one with larger objective.

    Args:
        archive_df (pd.DataFrame): dataframe of the archive
    """
    index_0_max = archive_df.loc[archive_df["index_0"] == archive_df["index_0"]
                                 .max()]["objective"].idxmax()
    index_0_min = archive_df.loc[archive_df["index_0"] == archive_df["index_0"]
                                 .min()]["objective"].idxmax()
    index_1_max = archive_df.loc[archive_df["index_1"] == archive_df["index_1"]
                                 .max()]["objective"].idxmax()
    index_1_min = archive_df.loc[archive_df["index_1"] == archive_df["index_1"]
                                 .min()]["objective"].idxmax()
    return [index_0_max, index_0_min, index_1_max, index_1_min]
