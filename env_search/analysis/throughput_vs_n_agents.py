...
def throughput_vs_n_agents(logdirs_plot: str, ax=None):
    ...
    all_runtime_dict = {}
    all_success_dict = {}
    if map_size in ["45x47", "57x58", "69x69"]:
        y_min = 0
        y_max = 20
    elif map_size in ["xxlarge"]:
        y_min = 0
        y_max = 15
    elif map_size in ["81x80", "93x91"]:
        y_min = 0
        y_max = 20
    elif map_size in ["33x57"]:
        y_min = 0
        y_max = 40
    elif map_size in ["50x86"]:
        y_min = 0
        y_max = 60
    elif map_size in ["140x500"]:
        y_min = 0
        y_max = 55
    else:
        y_min = 0
    ...
    for logdir_f in os.listdir(logdirs_plot):
        ...
        for sim_dir in os.listdir(results_dir):
            ...
            if os.path.exists(config_file) and os.path.exists(result_file):
                ...
                runtime = result.get("cpu_runtime", 0) # if not congested else 0
    ...
    all_success_vals = np.array(all_success_vals)
    success_rates = np.sum(all_success_vals,
                           axis=1) / all_success_vals.shape[1]
    # breakpoint()
