...
from env_search.warehouse.update_model.utils import (
    comp_uncompress_edge_matrix, comp_uncompress_vertex_matrix)
...
def plot_highway_edge_weights(map_np,
    ...
    optimized_wait_costs = comp_uncompress_vertex_matrix(
        map_np, weights[:n_valid_vertices], block_idxs, fill_value=0)
    ...
    fig_edges = plt.figure(figsize=(15, 13))
    ...
    cmap = "Reds"
    vmin = np.min(optimized_edge_weights)
    vmax = np.max(optimized_edge_weights)
    # vmin = 0
    # vmax = 2
    ...
    axs_edges[0][1].set_title('Right', fontsize=25)
    # fig.colorbar(cax1, ax=axs[0,1])

    cax2 = axs_edges[1][0].imshow(
        optimized_edge_weights[..., 1],
        cmap=cmap,
        vmin=vmin,
        vmax=vmax,
    )
    axs_edges[1][0].set_title('Up', fontsize=25)
    ...
    axs_edges[0][0].set_title('Left', fontsize=25)
    # fig.colorbar(cax3, ax=axs[0,0])

    cax4 = axs_edges[1][1].imshow(
        optimized_edge_weights[..., 3],
        cmap=cmap,
        vmin=vmin,
        vmax=vmax,
    )
    axs_edges[1][1].set_title('Down', fontsize=25)
    # fig.colorbar(cax4, ax=axs[1,1])

    cbar = axs_edges[1][1].cax.colorbar(cax4)
    ...
    if domain == "kiva":
        ax_wait.imshow(
            ...
            cmap=cmap,
            vmin=np.min(optimized_wait_costs),
            vmax=np.max(optimized_wait_costs),
        )
