...
def add_neighboaring_constraint(mdl, graph, objs, obj_t1, obj_t2):
    for i, neighbors in enumerate(graph):
        ...
        mdl.add_constraint(
            sum(adj_var_t2) >= objs[kiva_obj_types.index(obj_t1)][i])
...
def build_adj_list(n, m):
    """Build an adjacency list for the dynamics of the environment."""
...
def define_variables(mdl, n_nodes, obj_labels):
    ...
    for obj_label in obj_labels:
        curr_type = [
            mdl.integer_var(name='obj_{}_{}'.format(obj_label, i), lb=0, ub=1)
            for i in range(n_nodes)
        ]
...
def define_objective(mdl, adj, objs, env_np, add_movement):
    ...
    for cur_idx, cur_obj in enumerate(objs):
        ...
        for r in range(n):
            for c in range(m):
                i = r * m + c
                objects_in_graph.append((cur_obj[i], cur_idx == env_np[r, c]))
        objects.append((objects_in_graph, cost_move, cost_change))

    add_edit_distance(mdl, adj, objects, add_movement=add_movement)


def extract_repaired_env(solution, objs, n, m):
    """Extract repaired env from MILP solution"""
...
def partial_repair_env(
    ...
    """Partially repair the given unrepaired environment.

    Args:
        env_np (np.ndarray): unrepaired env of int format. Only has storage area
        time_limit (int, optional): time limit in seconds. Defaults to 60.
        add_movement (bool, optional): if True, use edit distance objective.
            Otherwise, use hamming distance objective. Defaults to True.
        max_n_shelf (int, optional): max number of shelves. Defaults to 25.
        min_n_shelf (int, optional): min number of shelves. Defaults to 10.
        seed (int, optional): seed of repairing. Defaults to 0.
        limit_n_shelf (bool, optional): if True, limit the number of shelves
            based on `max_n_shelf` and `min_n_shelf`. Defaults to True.
        n_threads (int, optional): number of threads to use. Defaults to 1.

    Returns:
        repair_env_int: repaired env of int format
    """
    n, m = env_np.shape
    n_nodes = n * m

    adj = build_adj_list(n, m)
    ...
    with Model(context=context) as mdl:
        ...
        # Define vars, we only need obj types in storage area
        objs = define_variables(mdl, n_nodes, kiva_obj_types[:3])
        ...
        define_objective(mdl, adj, objs, env_np, add_movement)
        ...
        new_env = extract_repaired_env(solution, objs, n, m)
...
def repair_env(
    ...
    """Repair given warehouse environment

    Args:
        env_np (np.ndarray): unrepaired env of int format
        agent_num (int): number of agents
        warm_envs_np (list[np.ndarray], optional): warm up solutions. Defaults
            to None.
        time_limit (int, optional): time limit in seconds. Defaults to 60.
        add_movement (bool, optional): if True, use edit distance objective.
            Otherwise, use hamming distance objective. Defaults to True.
        w_mode (bool, optional): if True the env has workstations. Defaults to
            False.
        max_n_shelf (int, optional): max number of shelves. Defaults to 25.
        min_n_shelf (int, optional): min number of shelves. Defaults to 10.
        seed (int, optional): seed of repairing. Defaults to 0.
    ...
    with Model(context=context) as mdl:
        ...
        # Define vars
        objs = define_variables(mdl, n_nodes, kiva_obj_types[:-1])
        ...
        # Extract repaired env if there is a solution
        new_env = extract_repaired_env(solution, objs, n, m)
...
def main(
    ...
    if partial_repair:
        repaired_env = partial_repair_env(
            env_np,
