...
def offline_run_nca_generator(
    ...
    global_opt_env = df.iloc[global_opt]["metadata"]["raw_metadata"][
        "repaired_env_str"]
    # global_opt_env_str = kiva_env_str2number(global_opt_env)
    # map_np_unrepaired = df.iloc[global_opt]["metadata"]["raw_metadata"][
    #     "unrepaired_env_int"]
    ...
    search_space = gin.query_parameter("WarehouseManager.search_space")
    if SearchSpace(search_space) == SearchSpace.LAYOUT_G_GRAPH:
        global_opt_nca_params = global_opt_nca_params[:num_params]
    ...
    if sim_score_with is not None and sim_score_with != "":
        ...
        sim_score_gen = cal_similarity_score(
            unrepaired_layout,
    ...
    visualize_kiva(
        hamming_repaired_env,
        ["repaired_nca_gen.png"],
        nca_process_dir,
    )
