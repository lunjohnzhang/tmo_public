...
def comp_compress_vertex_matrix(map_np, vertex_matrix, block_idxs):
    h, w = map_np.shape
    assert w * h == len(vertex_matrix)
    compressed_vertex_matrix = []
    for y in range(h):
        for x in range(w):
            pos = y * w + x
            if map_np[y, x] in block_idxs:
                continue
            compressed_vertex_matrix.append(vertex_matrix[pos])
    return compressed_vertex_matrix


def comp_compress_edge_matrix(map_np, edge_matrix, block_idxs):
    # edge matrix: h*w*[right,up,left,down]
    h, w = map_np.shape
    assert w * h * 4 == len(edge_matrix)
    ...
    for y in range(h):
        for x in range(w):
            ...
            if (x + 1) < w and map_np[y, x + 1] not in block_idxs:  # right
                compressed_edge_matrix.append(edge_matrix[4 * pos + 0])

            if (y - 1) >= 0 and map_np[y - 1, x] not in block_idxs:  # up
                compressed_edge_matrix.append(edge_matrix[4 * pos + 1])

            if (x - 1) >= 0 and map_np[y, x - 1] not in block_idxs:  # left
                compressed_edge_matrix.append(edge_matrix[4 * pos + 2])

            if (y + 1) < h and map_np[y + 1, x] not in block_idxs:  # down
                compressed_edge_matrix.append(edge_matrix[4 * pos + 3])
    return compressed_edge_matrix


def comp_uncompress_vertex_matrix(
    map_np,
    compressed_vertex_matrix,
    block_idxs,
    fill_value=np.nan,
...
def comp_uncompress_edge_matrix(
    map_np,
    compressed_edge_matrix,
    block_idx,
    fill_value=np.nan,
    ...
    for y in range(h):
        for x in range(w):
            if map_np[y, x] in block_idx:
                for i in range(4):
                    edge_matrix.append(fill_value)
            else:
                if (x + 1) < w and map_np[y, x + 1] not in block_idx:  # right
                    edge_matrix.append(compressed_edge_matrix[j])
                    j += 1
                else:
                    edge_matrix.append(fill_value)

                if (y - 1) >= 0 and map_np[y - 1, x] not in block_idx:  # up
                    edge_matrix.append(compressed_edge_matrix[j])
                    j += 1
                else:
                    edge_matrix.append(fill_value)

                if (x - 1) >= 0 and map_np[y, x - 1] not in block_idx:  # left
                    edge_matrix.append(compressed_edge_matrix[j])
                    j += 1
                else:
                    edge_matrix.append(fill_value)

                if (y + 1) < h and map_np[y + 1, x] not in block_idx:  # down
                    edge_matrix.append(compressed_edge_matrix[j])
                    j += 1
                else:
                    edge_matrix.append(fill_value)
