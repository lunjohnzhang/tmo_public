...
class WarehouseAugResnetOccupancy(nn.Module):
    ...
    def int_to_logits(self, envs: torch.Tensor) -> torch.Tensor:
        _, env_height, env_width = envs.shape
        outputs = self.int_to_no_crop(envs)
        return outputs[:, :, :env_height, :env_width]
...
class WarehouseAugResnetRepairedMapAndOccupancy(nn.Module):
    ...
    def int_to_logits(self, envs: torch.Tensor) -> torch.Tensor:
        _, env_height, env_width = envs.shape
        pred_repaired_map, occupancy = self.int_logits_to_no_crop(envs)
        return (pred_repaired_map[:, :, :env_height, :env_width],
                occupancy[:, :, :env_height, :env_width])

    def int_to_map_no_crop(self, envs: torch.Tensor) -> torch.Tensor:
        _, env_height, env_width = envs.shape
