...
class WarehouseMetadata:
    ...
    unrepaired_env_int: np.ndarray = None

    # Generated full layout in integer (repaired)
    repaired_env_int: np.ndarray = None

    # Generated full layout in str (repaired)
    repaired_env_str: List[List[str]] = None

    # Edge weights and wait costs (compressed version)
    edge_weights: np.ndarray = None
    wait_costs: np.ndarray = None

    objs: np.ndarray = None  # Objectives
    result_objs: np.ndarray = None
    ...
    # (n_eval, n_finished_tasks) 2D array
    finished_task_len: List[List[float]] = None
    ...
    # Average length of all possible tasks in the map
    all_task_len_mean: float = None
    ...
    avg_min_dist_to_ws: float = None
    avg_centroid_dist: float = None
...
class WarehouseResult:  # pylint: disable = too-many-instance-attributes
    ...
    @staticmethod
    def from_raw(
        raw_metadata: WarehouseMetadata,
        opts: dict = None,
    ):
        ...
        if opts["aggregation"] == "mean":
            agg_obj = maybe_mean(raw_metadata.objs)
            agg_result_obj = maybe_mean(raw_metadata.result_objs)
        elif opts["aggregation"] == "median":
            agg_obj = maybe_median(raw_metadata.objs)
            agg_result_obj = maybe_mean(raw_metadata.result_objs)
        else:
            raise ValueError(f"Unknown aggregation {opts['aggregation']}")

        agg_measures = WarehouseResult._obtain_measure_values(
            asdict(raw_metadata), opts["measure_names"])
