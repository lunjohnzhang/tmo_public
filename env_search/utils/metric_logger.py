...
class MetricLogger:
    ...
    def end_itr(self,
        ...
        if len(remaining_metrics) > 0:
            # Append 0 for the metrics that are not added
            if logger is not None:
                logger.info(
                    "The following metrics were not added this "
                    f"itr: {remaining_metrics}, adding 0 as placeholder")
                for metric in remaining_metrics:
                    self.add(metric, 0, logger)
    ...
    def add_post(
        self,
        name: str,
        values: Union[Sequence[float], Sequence[int]],
        use_zero: bool,
    ):
