...
class ManufactureManager:
    ...
    def __init__(
        ...
        env_width: int = gin.REQUIRED,
        env_height: int = gin.REQUIRED,
    ...
    def emulation_pipeline(self, sols):
        ...
        maps = np.array(sols).reshape(
            (n_maps, self.env_height, self.env_width)).astype(int)
    ...
    #             env, _ = manufactureNCA.generate(
    #                 self.seed_map_torch,
    #                 n_iter=self.nca_iter,
    #             )
    #             env = env.squeeze().cpu().numpy()
    #             envs.append(env)
    ...
    def add_experience(self, sol, result):
        ...
        input_env = result.manufacture_metadata["unrepaired_env_int"]
        repaired_env = result.manufacture_metadata["repaired_env_int"]
    ...
    def add_failed_info(sol, result) -> dict:
        ...
        failed_level_info = {
            "solution": sol,
            "unrepaired_env_int":
            result.manufacture_metadata["unrepaired_env_int"],
